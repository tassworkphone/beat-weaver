...
- **Chains — still the harder piece, no shortcut found yet.** Same missing-parser situation as arcs (`chains`/`chainsData` fields exist in the extracted official v4 JSON, unread), but unlike arcs, InfernoSaber's source has no chain-generation file at all — even this more automation-complete prior-art project treats chains as out of scope. No validated rule-based or model-native approach to point to yet; lowest priority of the three remaining map elements.
